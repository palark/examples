...
def lambda_handler(event, _):
    ...
    if writer_instance and writer_instance['DBInstanceClass'] != largest_instance_type:
        message = f"Scaling up writer instance: {writer_instance['DBInstanceIdentifier']}"
        print(message)
    ...
    if eligible_readers:
        ...
        message = f"Scaling up reader instance: {instance_to_scale_up['DBInstanceIdentifier']}"
        print(message)
        send_sns_alert(message)
    ...
    send_sns_alert("The process of modifying instances using a Lambda function in the cluster has been completed.")
...
def is_cluster_modifying(client, cluster_identifier):
    """
    checking if cluster already modifing
    """
...
def scale_instance(client, instance_identifier, new_instance_type):
    ...
    try:
        if new_instance_type is None:
            ...
            print(f"No suitable next instance type found for {instance_identifier}. Removing 'modifying' tag.")
            remove_tag_from_instance(rds_client, instance_identifier, 'modifying')
            return False  # The scaling operation was not performed

        # Continue with the scaling operation if a new instance type is available
        ...
        return True  # The scaling operation was successfully initiated
    except ClientError as e:
        error_message = f"Error during instance scaling: {e}"
        print(error_message)
...
def find_largest_instance_type(client, cluster_members):
    """
    find largest instance type (it's size will the goal of each instance in cluster)
    """
...
def find_eligible_readers_for_scale_up(client, cluster_members, largest_instance_type):
    """
    find eligible readers for scale up
    """
...
def select_random_instance(eligible_readers):
    """
    select random instance from eligible
    """
...
def add_modifying_tag(client, instance_identifier):
    ...
    try:
        ...
        error_messsage = f"Error adding 'modifying' tag to {instance_identifier}: {e}"
